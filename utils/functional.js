!function(global) {
  ...
  const spreadWithSpace = reduce((a, b) => `${a} ${b}`);
  const unique = uniqueBy(identity);
  const getter = curry((a, b) => a[b]);
  const isNotEmpty = coll => {
  ...
  const ascOrderByProperty = curry(
    (property, coll) => 
      coll.sort((a, b) => 
        a[property] < b[property] ? -1 : a[property] > b[property] ? 1 : 0)
  );

  const descOrderByProperty = curry(
    (property, coll) => 
      coll.sort((a, b) => 
        a[property] > b[property] ? -1 : a[property] < b[property] ? 1 : 0)
  );
  ...
  const Functional = {
    ...
    logf, spreadWithSpace, getter,
    isNotEmpty, isEmpty, flat,
    ascOrderByProperty, descOrderByProperty
  };
